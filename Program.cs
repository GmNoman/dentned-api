        ...
        public static void Main(string[] args)
        {
            ...
            app.MapPost("/api/appointments/book", async (AppointmentRequest request, DatabaseService dbService) =>
            {
                try
                {
                    ...
                    var newPatient = new Patient { FirstName = request.PatientFirstName, LastName = request.PatientLastName };
                    ...
                    return Results.Ok(new { success = true, message = "Appointment booked successfully", appointmentId, patientId });
                }
                catch (Exception ex)
                {
            ...
            app.MapGet("/api/appointments/available", async (DateTime date, DatabaseService dbService) =>
            {
                try
                {
                    using var connection = new SqlConnection(builder.Configuration.GetConnectionString("DentneDConnection"));
                    await connection.OpenAsync();
                    var query = @"SELECT appointments_from FROM appointments WHERE CAST(appointments_from AS DATE) = @Date AND appointments_from > GETDATE()";
                    using var command = new SqlCommand(query, connection);
            ...
            app.MapGet("/api/appointments", async (DatabaseService dbService) =>
            {
                try
                {
                    using var connection = new SqlConnection(builder.Configuration.GetConnectionString("DentneDConnection"));
                    await connection.OpenAsync();
                    var query = @"SELECT a.appointments_id, a.appointments_from, a.appointments_to, a.appointments_title, a.appointments_notes, p.patients_id, p.patients_name, p.patients_surname FROM appointments a INNER JOIN patients p ON a.patients_id = p.patients_id ORDER BY a.appointments_from DESC";
                    using var command = new SqlCommand(query, connection);
                    using var reader = await command.ExecuteReaderAsync();
            ...
            app.MapGet("/api/appointments/{id}", async (int id, DatabaseService dbService) =>
            {
                try
                {
                    using var connection = new SqlConnection(builder.Configuration.GetConnectionString("DentneDConnection"));
                    await connection.OpenAsync();
                    var query = @"SELECT a.appointments_id, a.appointments_from, a.appointments_to, a.appointments_title, a.appointments_notes, p.patients_id, p.patients_name, p.patients_surname FROM appointments a INNER JOIN patients p ON a.patients_id = p.patients_id WHERE a.appointments_id = @AppointmentId";
                    using var command = new SqlCommand(query, connection);
